  ...
  return (
    <div className="rounded-2xl border border-white/[0.07] bg-[#141414] text-white shadow-[0_18px_70px_rgba(0,0,0,0.18)]">
      <div className="flex items-center gap-1 border-b border-white/[0.07] px-2.5 py-2">
        <ToolbarButton label="Vet" onClick={() => runCommand('bold')}>
          B
        </ToolbarButton>
        <ToolbarButton label="Cursief" italic onClick={() => runCommand('italic')}>
          I
        </ToolbarButton>
        <ToolbarButton label="Bullet-lijst" onClick={() => runCommand('insertUnorderedList')}>
          <svg
            width="14"
            height="14"
            viewBox="0 0 24 24"
...
function ToolbarButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={[
        'flex h-8 w-8 items-center justify-center rounded-xl border border-white/[0.07] bg-white/[0.03] text-xs font-semibold text-white/55 transition-colors hover:border-[#facc15]/45 hover:bg-[#facc15] hover:text-black',
        italic ? 'italic' : '',
      ].join(' ')}
    >
      {children}
