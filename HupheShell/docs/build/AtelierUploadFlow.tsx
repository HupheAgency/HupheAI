...
export default function AtelierUploadFlow({
  ...
  if (!file) {
    return (
      <div className={[embedded ? 'h-full' : 'flex-1', 'flex items-center justify-center px-6'].join(' ')}>
        <div className="w-full max-w-md flex flex-col gap-3">
          ...
          <p className="text-center text-white/25 text-sm">
            of start een{' '}
            <button type="button" onClick={onBlankCanvas} className="text-white/60 hover:text-white transition-colors">
              leeg canvas
            </button>
          </p>
        </div>
  ...
  return (
    <div className={[embedded ? 'h-full' : 'flex-1', 'overflow-y-auto flex items-start justify-center px-6 py-10'].join(' ')}>
      <div className="w-full max-w-md space-y-6">
        ...
        {!isPresentationFile && !isOcrFile && (
          <Step index={2} label="Tekst" done={!!textMode} locked={!file}>
            <div className={['grid grid-cols-2 gap-2 transition-opacity', !file ? 'opacity-30 pointer-events-none select-none' : ''].join(' ')}>
              <OptionButton
                selected={textMode === 'manual'}
                disabled={!file}
                label="Zelf invullen"
                description="Jij bepaalt de tekst per slide"
                icon={<TextManualIcon />}
                onClick={() => onTextModeSelect('manual')}
              />
              <OptionButton
                selected={textMode === 'ai'}
                disabled={!file}
                label="AI schrijft tekst"
                description="AI maakt slides van jouw aantekeningen"
                icon={<AiIcon />}
                onClick={() => onTextModeSelect('ai')}
              />
            </div>
          </Step>
        )}
        ...
        {!isOcrFile && (
          <Step index={isPresentationFile ? 2 : 3} label="Beelden" done={!!imageMode} locked={!imageModeUnlocked}>
            <div className={['grid grid-cols-2 gap-2 transition-opacity', !imageModeUnlocked ? 'opacity-30 pointer-events-none select-none' : ''].join(' ')}>
              <OptionButton
                selected={imageMode === 'manual'}
                disabled={!imageModeUnlocked}
                label="Zelf invullen"
                description="Jij kiest zelf de afbeeldingen"
                icon={<ImageManualIcon />}
                onClick={() => onImageModeSelect('manual')}
              />
              <OptionButton
                selected={imageMode === 'ai'}
                disabled={!imageModeUnlocked}
                label="AI genereert beelden"
                description="AI maakt passende afbeeldingen per slide"
                icon={<AiIcon />}
                onClick={() => onImageModeSelect('ai')}
              />
            </div>
          </Step>
        )}

        <Step index={isOcrFile ? 2 : isPresentationFile ? 3 : 4} label="Template" done={!!templateClientId} locked={!templateUnlocked}>
          <div className={['relative transition-opacity', !templateUnlocked ? 'opacity-30 pointer-events-none select-none' : ''].join(' ')}>
            ...
            >
              <option value="" disabled>
                {clientsLoading ? 'Laden...' : clientsWithTemplate.length === 0 ? 'Geen templates beschikbaar' : 'Kies een template...'}
              </option>
