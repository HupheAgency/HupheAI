...
function historyReducer<T>(state: HistoryState<T>, action: HistoryAction<T>): HistoryState<T> {
  switch (action.type) {
    case 'PUSH': {
      ...
      if (newUndoStack.length > action.maxDepth) newUndoStack.shift()
...
export function useHistoryStack<T>(options?: UseHistoryStackOptions<T>): UseHistoryStackReturn<T> {
  ...
  const peek = useCallback(() => state.undoStack[state.undoStack.length - 1], [state.undoStack])

  return {
    push, undo, redo, clear, peek,
