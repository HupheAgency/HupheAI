...
interface BannerAnimatedPreviewProps {
  slides: { texts: { role: 'heading' | 'copy' | 'button'; value: string }[] }[]
  imageSrc: string
  width: number
  height: number
...
export default function BannerAnimatedPreview({
  ...
  const html = useMemo(() => {
    ...
    const frames = safeSlides.map((slide, index) => {
      ...
      const buttons = slide.texts.filter((text) => text.role === 'button' && text.value.trim())
      ...
      const buttonHtml = buttons.length > 0
        ? buttons.map((text) => `<div>${escapeHtml(text.value)}</div>`).join('')
        : ''
      ...
      return `
        <section class="frame" style="animation-delay: ${index * 3}s;">
          <div class="content">
            ...
            ${buttonHtml ? `<div class="button">${buttonHtml}</div>` : ''}
          </div>
        </section>
      `
    }).join('')
    ...
    <style>
      * { box-sizing: border-box; }
      ...
      .button {
        ...
        align-items: center;
        justify-content: center;
        margin-top: ${Math.max(8, baseFontSize * 0.58)}px;
        border-radius: 999px;
        background: #facc15;
        color: #111;
        padding: 0.62em 1.1em;
        font-size: ${baseFontSize * 0.56}px;
        line-height: 1;
        font-weight: 800;
        text-shadow: none;
      }
