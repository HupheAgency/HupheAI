...
interface TemplateWarning {
  type: 'no_sagetags' | 'no_image_slots' | 'unknown_font' | 'unsupported_element' | 'no_layouts'
  message: string
}
...
const warningLabels: Record<TemplateWarning['type'], string> = {
  no_sagetags: 'SageTags',
  no_image_slots: 'Afbeeldingen',
  unknown_font: 'Fonts',
  unsupported_element: 'Elementen',
  no_layouts: 'Layouts',
}
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-white text-sm font-medium">
              {layoutCount} layouts gevonden, {fieldCount} tekstvelden, {imageSlotCount} afbeeldingsslots
            </p>
            <p className={hasWarnings ? 'text-[#facc15]/80 text-xs mt-1' : 'text-green-400/80 text-xs mt-1'}>
              {hasWarnings ? `${warnings.length} waarschuwing${warnings.length === 1 ? '' : 'en'} gevonden` : 'Template ziet er goed uit'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <button
            type="button"
            onClick={onDismiss}
            aria-label="Templatecontrole sluiten"
